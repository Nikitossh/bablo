...
// Дата и время
    ...
    static String selectTotalAndAvgYearByCategory() {
        return  "select category.category as category, sum(value) as sum, truncate(sum(value)/12, 0) as avg " +
                "from costs inner join category on costs.category_id=category.id inner join date on costs.date_id=date.id " +
                "where year(date)='2018' " +
                "group by category order by category;";
    }

    static String selectTotalAndAngWholeYear() {
        return "select sum(value) as total, truncate(sum(value)/12, 0) as monthly " +
                "from costs inner join date on costs.date_id=date.id" +
                " where year(date)='2018';";
    }
